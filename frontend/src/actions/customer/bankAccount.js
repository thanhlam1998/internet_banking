...
function updateRemindList(remind_id, credit_number, remind_name, partner_code){
  return (dispatch) => {
    dispatch(request());
    BankAccountServices.updateRemindList(remind_id, credit_number, remind_name, partner_code)
    .then(
      res =>{
        dispatch(success(res))
      }
  ...
  function success(res){return{type: BankAccountConstants.UPDATE_REMIND_LIST_SUCCESS, payload: res}};
